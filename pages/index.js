...
export default function Home() {
	...
	async function loadNFTs() {
		/* create a provider and query for unsold market items */
		...
		/*
		*  map over items returned from the smart contract and format
	...
	async function buyNft(nft) {
		/* signs the transaction, uses Web3Provider and sign it */
	...
	if (loadingState === 'loaded' && !nfts.length) return (
		<h1 className="px-20 py-10 text-3xl">There are no NFTs in the marketplace</h1>)
	return (
							...
							<div className={'w-full flex flex-col gap-4'} key={category}>
								...
								<div className={'flex mb-4 gap-4'}>
									{groupedNfts[category].map((nft, i) => (
										...
										<div key={nft.name} className="border shadow rounded-xl overflow-hidden w-1/5" >
											<img src={nft.image}/>
											<div className="p-4 ">
												<p style={{height: '64px'}}
												...
												<div style={{height: '70px', overflow: 'hidden'}}>
													...
													<p className="text-red-400">{nft.category}</p>
												</div>
											</div>
